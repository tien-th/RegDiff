...
class BaseRegDiff(ABC):
    ...
    def initialize_model_optimizer_scheduler(self, config, is_test=False):
        ...
        if not is_test:
            ...
            img_size = self.config.data.dataset_config.image_size
            n_channels = self.config.data.dataset_config.channels
            ...
            self.smooth_optimizer = torch.optim.Adam(self.R_A.parameters(), lr=0.0001, betas=(0.5, 0.999))
                ...
                for train_batch in pbar:
                    ...
                    for i in range(len(self.optimizer)):
                        ...
                        if self.global_step % accumulate_grad_batches == 0:
                            ...
                            self.smooth_optimizer.step()
                            self.smooth_optimizer.zero_grad()
                            if self.scheduler is not None:
