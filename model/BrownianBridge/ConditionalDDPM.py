...
# TODO
    def p_losses(self, x0, y, context, t, noise=None):
        ...
        if self.loss_type == 'l1':
            ...
            recloss = F.mse_loss(objective, objective_recon, t)
        else:
            raise NotImplementedError()

        if self.objective == 'noise':
            x0_recon = self.predict_x0_from_noise(x_t, objective_recon, t, )
        else:
            raise NotImplementedError()       
    ...
    def predict_x0_from_noise(self, x_t , e_t, t): 
        alpha_bar = self.alpha_bar.clone().to(e_t.device)
        sqrt_alpha_bar = gather(alpha_bar, t) ** 0.5 
        var = 1 - gather(alpha_bar, t)
        x0 = (x_t - (var ** 0.5) * e_t )/sqrt_alpha_bar
        return x0
...
# TODO
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            _, x0_recon = self.get_x_prev_and_pred_x0(objective_recon, i, x_t, temperature=1., repeat_noise=False)
            if clip_denoised:
                x0_recon.clamp_(-1., 1.)
            ...
            x_t_hat = torch.cat([x_t, y], dim=1) 
            objective_recon = self.denoise_fn(x_t_hat, timesteps=t, context=context)
            x_prev, x0_recon = self.get_x_prev_and_pred_x0(objective_recon, i, x_t, temperature=1., repeat_noise=False)
